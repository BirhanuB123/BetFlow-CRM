...
export class WebsiteLeadsService {
  ...
  async capture(dto: WebsiteLeadCaptureDto) {
    ...
    await this.prisma.auditLog.create({
      data: {
        ...
        entityType: 'Lead',
        entityId: lead.id,
        newValues: { source: WEBSITE_SOURCE_NAME, email: dto.email ?? '', utm: dto.utmSource ?? '' },
      },
    });

    this.logger.log(`Website lead captured: ${lead.id} (${lead.firstName} ${lead.lastName})`);
  ...
  async stats() {
    const [total, today, thisWeek] = await Promise.all([
      this.prisma.lead.count({ where: { source: { name: WEBSITE_SOURCE_NAME } } }),
