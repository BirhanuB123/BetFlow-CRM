...
export class SocialLeadsService {
  ...
  verifyWebhook(mode: string, token: string, challenge: string): string {
    ...
    this.logger.warn(
      `Meta webhook verification failed. Received token: ${token}`,
    );
  ...
  // Per-event ingestion
  // -------------------------------------------------------
  private async ingestLeadgenEvent(
    ...
    const lastName =
      fields['last_name'] || nameParts.slice(1).join(' ') || 'Lead';
    ...
    this.logger.log(
      `Meta lead ingested: ${lead.id} from ${sourceName} (${event.campaign_name})`,
    );
    return lead;
  }
  ...
  private resolveSourceName(
    ...
    const combined =
      `${event.ad_name ?? ''} ${event.campaign_name ?? ''} ${fields['platform'] ?? ''}`.toLowerCase();
    return combined.includes('instagram')
      ? INSTAGRAM_SOURCE_NAME
      : FACEBOOK_SOURCE_NAME;
  }
  ...
  async stats() {
    const [facebook, instagram, thisWeek] = await Promise.all([
      this.prisma.lead.count({
        where: { source: { name: FACEBOOK_SOURCE_NAME } },
      }),
      this.prisma.lead.count({
        where: { source: { name: INSTAGRAM_SOURCE_NAME } },
      }),
      this.prisma.lead.count({
        where: {
          source: {
            name: { in: [FACEBOOK_SOURCE_NAME, INSTAGRAM_SOURCE_NAME] },
          },
