...
import { CheckCircle, Code2, Copy, ExternalLink, Globe, RefreshCw } from "lucide-react";
    ...
    >
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        ...
        <section className="rounded-xl border border-zinc-200 bg-white">
          ...
          <div className="p-5 space-y-4">
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-zinc-400 mb-1">Method</p>
              <span className="inline-block rounded-md bg-emerald-50 px-2 py-0.5 text-sm font-mono font-bold text-emerald-700">
                POST
              </span>
            </div>
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-zinc-400 mb-1">URL</p>
              <div className="flex items-center gap-2 rounded-lg border border-zinc-200 bg-zinc-50 px-3 py-2">
                <code className="flex-1 truncate text-sm text-zinc-800">{apiUrl}</code>
                <button
                  onClick={() => copy(apiUrl)}
            ...
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-zinc-400 mb-1">Auth</p>
              <p className="text-sm text-zinc-600">
                No authentication required — this endpoint is public. Protect it with CORS and rate-limiting at your reverse proxy.
              </p>
            </div>
            <div className="flex flex-wrap gap-2 pt-1">
              {["firstName *", "lastName *", "email", "phone", "company", "message", "utmSource", "utmMedium", "utmCampaign"].map((f) => (
                <span key={f} className={`rounded-full px-2 py-0.5 text-xs font-mono ${f.endsWith("*") ? "bg-blue-50 text-blue-700" : "bg-zinc-100 text-zinc-600"}`}>
                  {f}
                </span>
              ))}
            </div>
        ...
        <section className="rounded-xl border border-zinc-200 bg-white">
          ...
          <div className="border-t border-zinc-200 px-5 py-4">
            <p className="mb-2 text-sm font-semibold text-zinc-700">Success response</p>
      ...
      <section className="mt-6 rounded-xl border border-zinc-200 bg-white">
        <div className="border-b border-zinc-200 px-5 py-4">
          ...
          <p className="text-sm text-zinc-500">Connect your website, landing page, or chatbot in 3 steps.</p>
        </div>
        <ol className="divide-y divide-zinc-100">
          {[
            { step: "1", title: "Add a contact form to your website", desc: "Include firstName, lastName, email, and phone fields. Add UTM hidden fields to track campaign source." },
            { step: "2", title: "Submit the form to the capture endpoint", desc: "On submit, send a POST request with a JSON body to the URL above. No API key needed." },
            { step: "3", title: "Monitor leads in BetFlow", desc: "New leads appear instantly in the Leads module tagged with the 'Website' source. Assign them to agents." },
          ].map((item) => (
