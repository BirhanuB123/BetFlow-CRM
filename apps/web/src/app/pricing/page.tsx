      ...
      <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
        <div className="max-w-3xl">
          ...
          <h1 className="mt-3 text-4xl font-semibold tracking-tight">
            Plans for every real estate sales operation.
          </h1>
          <p className="mt-4 text-base leading-7 text-zinc-600">
            Start with the seeded demo, then choose the plan that fits your
            tenant size, automation volume, and integration requirements.
          </p>
        </div>
        ...
        <div className="mt-8 grid gap-4 lg:grid-cols-4">
          {pricingPlans.map((plan) => (
            <section
              key={plan.name}
              className={`rounded-lg border bg-white p-5 ${plan.featured ? "border-zinc-950 shadow-sm" : "border-zinc-200"}`}
            >
              ...
              <Link
                href="/auth"
                className="mt-6 inline-flex h-10 w-full items-center justify-center rounded-md bg-zinc-950 px-4 text-sm font-medium text-white"
              >
                Start demo
              </Link>
            </section>
