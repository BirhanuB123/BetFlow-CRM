...
export function CustomersView() {
  ...
  const filteredCustomers = useMemo(() => {
    return customers.filter((customer) => {
      ...
      if (selectedFilters.size > 0) {
        for (const filterItem of selectedFilters) {
          switch (filterItem) {
            case "Email":
  ...
  return (
    <div className="flex h-[calc(100vh-210px)] min-h-[500px] gap-0 overflow-hidden rounded-md border border-zinc-200 bg-white shadow-sm">
